##include "vehiculo.h"   // implementación de la clase Vehiculo
#include <iostream>     // para usar cout

using namespace std;


// CONSTRUCTOR VACÍO

Vehiculo::Vehiculo() {
    // inicializa el vehículo sin datos

    marca = "";
    modelo = "";
    anio = 0;
    placas = "";
    kilometraje = 0;
}


// CONSTRUCTOR CON PARÁMETROS

Vehiculo::Vehiculo(string marca, string modelo, int anio, string placas, int kilometraje) {
    // crea un vehículo con datos iniciales

    this->marca = marca;
    this->modelo = modelo;
    this->anio = anio;
    this->placas = placas;
    this->kilometraje = kilometraje;
}


// ACTUALIZAR PLACAS

void Vehiculo::actualizarPlacas(string nuevasPlacas) {
    placas = nuevasPlacas; // cambia las placas
}


// ACTUALIZAR KILOMETRAJE

void Vehiculo::actualizarKilometraje(int nuevoKilometraje) {
    kilometraje = nuevoKilometraje; // cambia el kilometraje
}


// MOSTRAR INFORMACIÓN

void Vehiculo::mostrarInformacion() {

    cout << "Marca: " << marca << endl;
    cout << "Modelo: " << modelo << endl;
    cout << "Anio: " << anio << endl;
    cout << "Placas: " << placas << endl;
    cout << "Kilometraje: " << kilometraje << endl;
}
